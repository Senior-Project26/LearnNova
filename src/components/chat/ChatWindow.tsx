...
export default function ChatWindow({ mode = "full", initialPrompt }: ChatWindowProps) {
  ...
  useEffect(() => {
    ...
    if (hasSentInitialPrompt) return;
    if (messages.length > 0) return;
    if (state.isStreaming) return;
    send(initialPrompt, { explainLike5: eli5, detailed, tutorNoAnswer });
    setHasSentInitialPrompt(true);
  }, [initialPrompt, messages.length, state.isStreaming, send, eli5, detailed, tutorNoAnswer, hasSentInitialPrompt]);
